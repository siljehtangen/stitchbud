...
import com.stitchbud.util.getOrNull
...
class FriendshipService(
    ...
    fun upsertProfile(userId: String, email: String, displayName: String?): UserProfileDto {
        val profile = (userProfileRepository.findById(userId).getOrNull()
            ?: UserProfile(userId = userId, email = email)).also {
            it.email = email
            displayName?.let { name -> it.displayName = name }
    ...
    fun sendFriendRequest(requesterId: String, targetEmail: String): FriendDto {
        val requester = userProfileRepository.findById(requesterId).getOrNull()
            ?: throw BadRequestException("Profile not found. Please try again.")
        ...
        val target = userProfileRepository.findByEmail(targetEmail)
            ?: throw NotFoundException("No user found with email $targetEmail")

        friendshipRepository.findBetween(requesterId, target.userId)?.let { f ->
            ...
            friendshipRepository.save(f.apply { status = FriendshipStatus.ACCEPTED })
    ...
    fun getFriends(userId: String): List<FriendDto> {
        fun Friendship.friendId() = if (requesterId == userId) recipientId else requesterId
        val friendships = friendshipRepository.findAcceptedFriendships(userId)
        val profiles = userProfileRepository.findAllById(friendships.map { it.friendId() }).associateBy { it.userId }
        return friendships.map { f ->
            val friendId = f.friendId()
            FriendDto(f.id, friendId, profiles[friendId]?.displayName, profiles[friendId]?.email ?: "")
        }
    }

    @Transactional(readOnly = true)
    fun getPendingRequests(userId: String): List<FriendRequestDto> {
        val requests = friendshipRepository.findByRecipientIdAndStatus(userId, FriendshipStatus.PENDING)
        val profiles = userProfileRepository.findAllById(requests.map { it.requesterId }).associateBy { it.userId }
        return requests.map { f ->
            val profile = profiles[f.requesterId]
    ...
    fun acceptFriendRequest(friendshipId: Long, userId: String): FriendDto {
        val friendship = friendshipRepository.findById(friendshipId).getOrNull()
        ...
        friendshipRepository.save(friendship.apply { status = FriendshipStatus.ACCEPTED })
        val profile = userProfileRepository.findById(friendship.requesterId).getOrNull()
    ...
    private fun verifyFriendship(requesterId: String, friendUserId: String) {
        val friendship = friendshipRepository.findBetween(requesterId, friendUserId)
            ?: throw BadRequestException("You are not friends.")
